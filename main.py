helados = []  # Lista para almacenar los helados - CORREGIDO: SE CAMBIÓ 'heladoos' POR 'helados'
contador_id = 1  # Contador para asignar IDs únicos - CORREGIDO: SE CAMBIÓ 'contadore_id' POR 'contador_id'
...
while True:
    ...
    if opcion == "1":  # Agregar un helado
        ...
        precio = input("Ingrese el precio del helado: ")  # CORREGIDO: SE CAMBIÓ 'precioo' POR 'precio'
        
        if precio.isdigit():
            precio = float(precio)  # CORREGIDO: SE DEFINIÓ CORRECTAMENTE LA VARIABLE 'precio'
            helado = {"id": contador_id, "nombre": nombre, "descripcion": descripcion, "precio": precio}  # CORREGIDO: SE CAMBIÓ 'heladoo' A 'helado'
            helados.append(helado)  # CORREGIDO: SE CAMBIÓ 'haledo' A 'helado'
            contador_id += 1  # CORREGIDO: SE CAMBIÓ 'contadore_id' A 'contador_id'
            print("Helado agregado correctamente.")
        else:
            print("Error: El precio debe ser un número.")

    elif opcion == "2":  # Ver lista de helados
        if len(helados) == 0:  # CORREGIDO: SE CAMBIÓ 'heladoos' A 'helados'
            print("No hay helados registrados.")
        else:
            print("\nLista de Helados:")
            for helado in helados:
                print(f"ID: {helado['id']}, Nombre: {helado['nombre']}, Descripción: {helado['descripcion']}, Precio: ${helado['precio']}")  # CORREGIDO: SE CORRIGIERON CLAVES MAL ESCRITAS CON F-STRING
        ...
        if id_modificar.isdigit():
            ...
            for helado in helados:
                if helado["id"] == id_modificar:
                    nuevo_nombre = input("Nuevo nombre (deje en blanco para no cambiar): ")  # CORREGIDO: SE ELIMINÓ DOBLE SIGNO IGUAL '=='
        ...
        if id_eliminar.isdigit():
            ...
            for helado in helados:
                if helado["id"] == id_eliminar:
                    helados.remove(helado)  # CORREGIDO: SE CAMBIÓ 'heladoo' POR 'helado'
